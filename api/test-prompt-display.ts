...
Matching Rules:
1. The "Topic" field may contain multiple distinct tags separated by commas (e.g. "Printer,Display").
2. If the query has spelling mistakes, mentally correct them first (e.g., "printor" -> "printer", "repots" -> "reports", "dispay" -> "display").
3. Find ALL videos that are relevant to the core concepts in the query. Do NOT filter out videos based on extra words like 'ECO D'.
...
5. If NO videos match precisely, return an empty array: []`;
  ...
  await runQuery("display settings");
}
testGroq();
