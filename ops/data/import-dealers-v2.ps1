...
# Targets: poornasree-v4 VPS (65.20.72.131) via SSH alias
# SSH key : ~/.ssh/poornasreeAI2
...
Write-Host " PoornasreeAI - Import Dealers (v2 / Docker)" -ForegroundColor Cyan
Write-Host "============================================" -ForegroundColor Cyan
Write-Host ""

# Node.js script — runs inside API container (Prisma + bcrypt already present)
$nodeScript = @'
...
const DEALERS = [
  ...
  {n:'DAIRY CONSULTING LTD',       s:'Kenya',              p:null,     c:'Nairobi'},
...
async function main() {
  ...
  for (const d of DEALERS) {
    ...
    try {
      ...
      if (existing) {
        process.stdout.write('  ~ ' + d.n + ' -- already exists, skipped\n');
        skipped++;
        continue;
      }
      let pincodeId = null;
      ...
      await prisma.user.create({
        data: { email, passwordHash: hash, firstName: d.n, role: 'dealer', pincodeId }
      });
      process.stdout.write('  + ' + d.n + ' (' + email + ')\n');
      created++;
    } catch (e) {
      process.stdout.write('  ! ' + d.n + ' -- ' + e.message + '\n');
      errors++;
    }
  }
  process.stdout.write('\n--- Done: ' + created + ' created, ' + skipped + ' skipped, ' + errors + ' errors ---\n');
  process.stdout.write('Default password for all new dealers: ' + PWD + '\n');
  await prisma.$disconnect();
}
main().then(() => process.exit(0)).catch(e => { console.error(e); process.exit(1); });
'@

Write-Host "[1/1] Running dealer import inside API container on VPS..." -ForegroundColor Yellow

# Encode to base64 so single-quotes inside the node script don't break the shell command
...
& ssh poornasree-v4 $cmd

Write-Host ""
Write-Host "============================================" -ForegroundColor Green
Write-Host " Import complete!" -ForegroundColor Green
Write-Host "============================================" -ForegroundColor Green
Write-Host " Default password : Dealer@2026" -ForegroundColor Cyan
