...
check_local_env_is_not_dev_only() {
  ...
  if [[ "$warned" -eq 1 ]]; then
    ...
    echo "   Expected value: <your production JWT_SECRET>"
    echo ""
  fi
}
...
run_deploy() {
  local m="$1"
  local cmd="cd '$REMOTE_DIR' && git remote set-url origin https://github.com/pydart-hub/PoornasreeAI.git && env SKIP_OLLAMA=1 bash -c 'if [ -f scripts/deploy/deploy.sh ]; then bash scripts/deploy/deploy.sh \"$@\"; else bash deploy.sh \"$@\"; fi' _ $m"
