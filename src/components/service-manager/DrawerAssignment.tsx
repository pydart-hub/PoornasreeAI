...
import { getAssignmentMode, DEALER_RESPONSE_LABELS } from "./assignmentMode";
...
export function DrawerAssignment({ ticket, engineers, dealers: _dealers, assistants, assigningId, assigningDealerId, onAssignEngineer, onAssignDealer, onAssignAssistant }: DrawerAssignmentProps) {
  ...
  const showEngineerAssign = assignmentMode === "engineer" && !engineer;
  const showMatchedDealerAssign = assignmentMode === "matched-dealer" && !ticket.assignedDealer;
  const isDealerOrSerialTicket = !isClosed && (showMatchedDealerAssign || !!ticket.assignedDealer || !!ticket.machineSerialNumber?.trim());
  ...
  return (
    <div className="px-5 py-4 border-t border-line dark:border-line-dark">
      ...
      {/* ── Dealer section ── */}
      {!isClosed && (showMatchedDealerAssign || ticket.assignedDealer) && (
        <div className={cn(showEngineerAssign || engineer ? "mt-4 pt-3 border-t border-line dark:border-line-dark" : "")}>
          <h4 className="text-[10px] font-semibold text-content-tertiary dark:text-content-dark-tertiary uppercase tracking-wider mb-2">
            Dealer Assignment
          </h4>

          {showMatchedDealerAssign && ticket.dealerId && ticket.dealer && (
            <div className="p-2.5 bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-500/20 rounded-lg">
              <p className="text-[10px] font-semibold text-emerald-700 dark:text-emerald-400 uppercase tracking-wider mb-1.5">
                Matched dealer (Passtest serial)
              </p>
          ...
          {ticket.assignedDealer && (
            <div className="flex items-center justify-between gap-2">
              ...
              {ticket.dealerResponse && (
                <span className="text-[10px] font-semibold px-2 py-0.5 rounded-full bg-violet-100 text-violet-700 dark:bg-violet-500/15 dark:text-violet-300 shrink-0">
          ...
          {!engineer && isDealerOrSerialTicket && (
            <div className="mt-3 p-3 bg-amber-50/70 dark:bg-amber-500/10 border border-amber-200/80 dark:border-amber-500/20 rounded-xl space-y-2.5">
              <div className="flex items-center justify-between gap-2">
                <div className="min-w-0">
                  <p className="text-xs font-semibold text-amber-900 dark:text-amber-300 flex items-center gap-1.5">
                    ...
                    Backup Option: Assign to Engineer
                  </p>
                  <p className="text-[10px] text-amber-700/85 dark:text-amber-400/85 mt-0.5 leading-snug">
                    {ticket.assignedDealer
                      ? "Dealer assigned. You can reassign to an internal service engineer instead."
                      : "Serial matched to dealer. If dealer is unavailable, assign an engineer directly."}
                  </p>
                </div>
                {!reassignOpen && (
                  <button
                    type="button"
                    onClick={() => { setConfirmEng(null); setReassignOpen(true); }}
                    className="px-2.5 py-1.5 rounded-lg text-xs font-semibold bg-amber-600 hover:bg-amber-700 text-white transition-colors shrink-0 shadow-sm flex items-center gap-1"
                  >
              ...
              {/* Inline engineer selector when opened from backup card */}
              {reassignOpen && !confirmEng && (
                <div className="pt-2 border-t border-amber-200/60 dark:border-amber-500/20 space-y-2">
                  <div className="flex items-center justify-between">
                    <p className="text-xs font-semibold text-content-secondary dark:text-content-dark-secondary">
                    ...
                    <button
                      ...
                      className="text-xs text-content-tertiary dark:text-content-dark-tertiary hover:text-content-secondary dark:hover:text-content-dark-secondary"
                    >
                      Cancel
                    </button>
                  </div>
                  {matched.length === 0 ? (
                    <p className="text-xs text-amber-700 dark:text-amber-300 bg-white/70 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-500/30 rounded-lg p-2 text-center">
                      {ticketPincode
                        ? `No engineer covers zone ${ticketPincode.code} — assign a pincode to an engineer in the Team tab first`
                        : "No zone on ticket — set a pincode on the ticket before assigning an engineer"}
                    </p>
                  ) : (
                    <div className="border border-line dark:border-line-dark rounded-lg overflow-hidden max-h-48 overflow-y-auto bg-surface-card dark:bg-surface-dark-card">
                      {matched.map(eng => (
                        <button
                          key={eng.id}
                          type="button"
              ...
              {/* Confirmation view when engineer selected in backup card */}
              {confirmEng && (
                <div className="pt-2 border-t border-amber-200/60 dark:border-amber-500/20 space-y-2">
                  <div className="bg-white/80 dark:bg-amber-950/40 border border-amber-300 dark:border-amber-500/30 rounded-lg p-2.5 space-y-1">
                    ...
                    <p className="text-[10px] text-amber-700 dark:text-amber-400">
                      {ticket.assignedDealer
                        ...
                        : ticket.dealer
                          ...
                          : "Direct engineer assignment (backup option)"}
                    </p>
                  </div>
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      onClick={handleConfirm}
                      disabled={assigningId === ticket.id}
                      className="flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-semibold bg-primary text-white hover:bg-primary-hover disabled:opacity-50 transition-colors shadow-sm"
                    >
                      {assigningId === ticket.id ? <Loader2 className="w-3 h-3 animate-spin" /> : <UserCheck className="w-3 h-3" />}
                      Confirm Assignment
                    </button>
